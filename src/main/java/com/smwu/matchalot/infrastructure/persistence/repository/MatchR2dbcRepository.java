...
public interface MatchR2dbcRepository extends R2dbcRepository<MatchEntity, Long> {
    @Query("SELECT * FROM matches WHERE requester_id = :userId OR partner_id = :userId ORDER BY created_at DESC")
    ...
    @Query("SELECT COUNT(*) > 0 FROM matches WHERE " +
            "((requester_id = :userId1 AND partner_id = :userId2) OR " +
            " (requester_id = :userId2 AND partner_id = :userId1)) AND " +
            "status IN ('PENDING', 'ACCEPTED')")
    Mono<Boolean> existsPendingMatchBetween(Long userId1, Long userId2);

    @Query("SELECT COUNT(*) > 0 FROM matches WHERE " +
            "(requester_material_id = :materialId OR partner_material_id = :materialId) AND " +
            "status IN ('PENDING', 'ACCEPTED')")
    Mono<Boolean> existsActiveMatchForMaterial(Long materialId);

    // 특정 족보 간 매칭
    @Query("SELECT * FROM matches WHERE " +
            "((requester_material_id = :materialId1 AND partner_material_id = :materialId2) OR " +
            " (requester_material_id = :materialId2 AND partner_material_id = :materialId1)) " +
    ...
    // 통계
    @Query("SELECT COUNT(*) FROM matches WHERE (requester_id = :userId OR partner_id = :userId) AND status = :status")
    Mono<Long> countByUserIdAndStatus(Long userId, String status);



    @Query("SELECT COUNT(*) FROM matches WHERE requester_id = :userId OR partner_id = :userId")
